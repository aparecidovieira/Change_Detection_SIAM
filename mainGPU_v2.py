...
def load_divided_(dataset_dir):
    ...
    names = train_input_names + val_input_names
    train_input_names_ = []
    val_input_names_ = []
    
    for i,name in enumerate(train_input_names):
        img = cv2.imread(name, -1)
        if img.shape == (256, 512, 3):
            train_input_names_.append(name)
    for i,name in enumerate(val_input_names):
        img = cv2.imread(name, -1)
        if img.shape == (256, 512, 3):
            val_input_names_.append(name)
#             del train_input_names[i]
    return train_input_names_, train_output_names, val_input_names_, val_output_names, test_input_names, test_output_names
...
#     cwd = os.getcwd()
    print(dataset_dir)
    train_input_names = glob.glob(dataset_dir + '/train/input1/*')
    train_output_names = glob.glob(dataset_dir + '/train/mask/*')
    val_input_names = glob.glob(dataset_dir + '/val/input1/*')
    val_output_names = glob.glob(dataset_dir + '/val/mask/*')
...
def prepare_data(dataset_dir=args.dataset, type=args.load_data):
    if type:
        if 'SMC_Dataset' in args.dataset:
            load_func = load_divided
        else:
            load_func = load_divided_
        train_input_names, train_output_names, val_input_names, val_output_names, test_input_names, test_output_names = load_func(dataset_dir)
    else:
...
def load_image(path, change=True):
#     print(path)
    image = cv2.cvtColor(cv2.imread(path,-1), cv2.COLOR_BGR2RGB)
    if change and 'input1' in path: 
        image1 = cv2.cvtColor(cv2.imread(path.replace('input1', 'input2'),-1), cv2.COLOR_BGR2RGB)
...
class_names_list,label_values=['background', 'change'],[(0, 0, 0),(255,255,255)]
#helpers.get_label_info(os.path.join(args.dataset, "class_dict.csv"))
class_names_string = ""
...
#                     _loss = focal_loss(network, output_A[gpu_id])
                    loss_l.append(_loss)
...
model_checkpoint_name = check+"/latest_model_" + args.model + "_" + os.path.basename(args.dataset) + ".ckpt"
    ...
    for epoch in range(0, args.num_epochs):
        ...
        for i in range(num_iters):
            ...
            for j in range(args.batch_size):
                ...
                output_image = load_image(train_output_names[id], change=False)[:, :, :]
            ...
            # Do the training
            _,current=sess.run([opt,loss],feed_dict={input:input_image_batch,output:output_image_batch, keep_prob:1.0})#, wavelet1:wave1, wavelet2:wave2, wavelet3: wave3, wavelet4: wave4, keep_prob:0.25 })
        ...
        for ind in val_indices:
            input_image = np.float32(load_image(val_input_names[ind])[:,:])
            #input_image[:, :, 0] -= 103.939
            #input_image[:, :, 1] -= 116.779
            #input_image[:, :, 2] -= 123.68

            input_image = np.expand_dims(input_image, axis=0)/255.0
            gt = load_image(val_output_names[ind], change=False)[:, :, :]
#             gt = gt/255.0
...
#             cv2.imwrite("%s/%04d/%s_aux_pred.png"%(check,epoch, file_name),cv2.cvtColor(np.uint8(out_vis_image), cv2.COLOR_RGB2BGR))
            ...
            img1 = cv2.imread(val_output_names[ind], -1)
#             shutil.copy(val_output_names[ind], "%s/%04d/%s_gt.png"%(check,epoch, file_name))
            if  os.path.isfile(val_output_names[ind].replace('input1', 'input2')):
#                 img1 = cv2.imread(val_output_names[ind], -1)
                img2 = cv2.imread(val_output_names[ind].replace('input1', 'input2'), -1)
                img1 = np.concatenate((img1, img2), axis=1)
            cv2.imwrite("%s/%04d/%s_gt.png"%(check,epoch, file_name),img1) 
        ...
        print("Class average score" % (class_avg_scores))
#         for index, item in enumerate(class_avg_scores):
#             print("%s = %f" % (class_names_list[index], item))
    ...
    for ind in range(len(val_input_names)):
        ...
        gt = load_image(val_output_names[ind])[:, :]
        gt = helpers.reverse_one_hot(helpers.one_hot_it(gt, label_values))
